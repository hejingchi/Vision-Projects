...
import cv2
from model_resnet import *  # 导入 ResNet 和 BasicBlock
...
def load_model(model_folder="Model_ResNet", model_date=None, model_version=None, prefix="No_Flip_Rotation", p=0.8):
    """
    加载模型权重。

    参数:
        model_folder: 模型根目录
        model_date: 日期文件夹，如 "20260605"。为 None 时自动选最新日期
    ...
    """
    # 创建 ResNet-18 网络
    net = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=6)
...
# 方式2：指定日期和版本
net, info = load_model(model_date="20260605", model_version=1, prefix="No_Flip_Rotation")

# 方式3：只指定日期，自动选最新版本
# net, info = load_model(model_date="20260516")

# 方式4：不同 prefix（不同增强方案对比）
# net, info = load_model(prefix="Flip_Rotation")
