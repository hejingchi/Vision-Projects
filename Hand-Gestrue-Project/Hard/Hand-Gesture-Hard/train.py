...
def train(p = 0.8): # 训练数据集的同时进行模型生成
    ...
    # 2. 规定每个阶段残差块的数量（这里以 ResNet-18 的配置 [2, 2, 2] 为例）
    # 如果想用 ResNet-34 的配置，可以改为 [3, 4, 6]
    blocks_sum = [3, 4, 6]
    use_ResNet = False # 选择ResNet方法
    # 3. 传入参数创建网络实例
    if use_ResNet:
    ...
    if not transform:
        if use_ResNet:
            save_model(net, "Model_ResNet", "No_Flip_Rotation", p, 'pth', True)  # 保存模型
        else:
            save_model(net, "Model", "No_Flip_Rotation", p, 'pth', True)  # 保存模型
    else:
        if use_ResNet:
            save_model(net, "Model_ResNet", "Flip_Rotation", p, 'pth', True)  # 保存模型
        else:
            save_model(net, "Model", "Flip_Rotation", p, 'pth', True)  # 保存模型
